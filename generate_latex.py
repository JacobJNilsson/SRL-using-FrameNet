...
def main():
    ...
    for subdir, dirs, files in os.walk(rootdir):
        ...
        if descr_file in files and malt_id_file in files and malt_label_file in files and spacy_id_file in files and spacy_label_file in files:
            ...
            if 'Testing all features not generated by parser.' in first_line:
                # feature_list = get_features(df)
                ...
                # first_line = f'Features: {feature_list}'
...
def get_features(file):
    for line in file:
        if not 'features' in line:
            pass
        else:
            break
    features = re.search('features={\'(.+?)\'}.', line).group(1).split(',')
    return features
